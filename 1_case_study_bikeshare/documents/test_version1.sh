...
organize_csv_columns(){
    ...
    do
        ...
        cat curheader | paste -s -d "," | sed 's/,/|,|/g' | sed 's/^/|/g' | sed 's/$/|/g' | tr "," '\n' > test
        mv test curheader
        
        # ---------------------------
        # Create null column : Get the number of rows of the current csv
        export col_len=$(cat $filename | wc -l)
        for i in $( seq 2 $col_len )
        do
            # Create an nan column for header values of tot_header that are not in header2
            echo "NAN" >> nullcol
        done
        # ---------------------------
        
        # ---------------------------
        # Select the primary_key_col
        unset col_loc_in_header2
        c=1
        while [ -f $col_loc_in_header2 ]
        do
            # Go down the list
            export primary_key_col_name=$(cat primary_key_list | head -n $c | tail -n $(($c-($c-1))) | tr -d '\r')
            export primary_key_col_bars1=$(echo $primary_key_col_name | sed 's/^/|/g' | sed 's/$/|/g')
            export col_loc_in_header2=$(grep -n -F $primary_key_col_bars1 curheader)
            c=$((c+1))
        done
        
        export colnum=$(echo $col_loc_in_header2 | cut -d ':' -f 1)
        cat $filename | cut -d ',' -f $colnum > primary_key_col
        # ---------------------------
        
        
        # There should only be one match, if a match exists
        export n=$(cat tot_header | wc -l)
        echo "n: "
        echo $n
        ...
        do
          ...
          # Do not include primary_key_col_name in the main table
          if [[ $genheader != $primary_key_col_name ]]; then

              # add bars to define the word
              export genheader_bars1=$(echo $genheader | sed 's/^/|/g' | sed 's/$/|/g')
              echo "genheader_bars1:"
              echo $genheader_bars1

              # Count fixed strings (-F) from a file per line
              export loc_in_curheader=$(grep -n -F $genheader_bars1 curheader)
              echo "loc_in_curheader:"
              echo $loc_in_curheader
              
              # ---------------------------
              if [ ! -f $loc_in_curheader ]; then
                echo "header2 was found in tot_header : add column to new csv file"
                
                export colnum=$(echo $loc_in_curheader | cut -d ':' -f 1)
                echo "colnum:"
                echo $colnum
              
                cat $filename | cut -d ',' -f $colnum > col
                echo "Confirmation of column selection: "
                cat col | head -n 5
              else
                # No match found, insert null column
                echo "header2 was NOT found in tot_header : add null column to new csv file"
                
                # Append header name (genheader) to top of column : append one row to many rows
                echo $genheader > tempgen
                cat tempgen nullcol > col
                rm tempgen
                echo "Confirmation of null column creation: "
                cat col | head -n 5
              fi
              # ---------------------------
              
              # ---------------------------
              # Only 11 columns can be concatenated into a single csv file
              # ---------------------------
              # Add column to processed_${filename}_${fnum} : it should be $r%12, but make it $r%11 so that one column remains for a [primary key] to join all the datasets
              export modulus=$(echo "$r%11" | bc)
              echo "modulus:"
              echo $modulus
              ...
              if [[ $r -eq 1 ]];then
                  ...
                  echo '' >> processed_${filename}_${fnum}
                  echo "processed_${filename}_${fnum} file is empty : do not add delimeters"
                  paste processed_${filename}_${fnum} col -d "" > tempjoin
              ...
              mv tempjoin processed_${filename}_${fnum}
              echo "Confirmation of processed_${filename}_${fnum} modification: "
              cat processed_${filename}_${fnum} | head -n 5
              # ---------------------------
          fi
        ...
        # ---------------------------
        # Concatenate [primary key] to each of the tables
        # ---------------------------
        for r in $(seq 1 $fnum)
        do  
            paste primary_key_col processed_${filename}_${r} -d "," > tempjoin
            mv tempjoin processed_${filename}_${r}
        done
...
initial_setup(){
    ...
    # get main path
    cd /home/oem2/Documents/COURS_ONLINE/Spécialisation_Google_Data_Analytics/3_Google_Data_Analytics_Capstone_Complete_a_Case_Study
    export cur_path=$(pwd)
...
exact_match(){
    ...
    # Logic : After narrowing corresponding matches between f_row(a word per row of header2) and tot_header using substrings, perform: 0. Exact match, 1. if there is not an exact match use the model. 
    
    cat $1 > temp2 

    r=1
    for header2_word in $(cat temp2)
    do 
        flag=0
        ...
        if [[ $flag == "1" ]]; then
            ...
            export to_replace=$(cat $1 | head -n $r | tail -n $(($r-($r-1)))| tr -d '\r')
            export to_replace_with=$(echo "")
            
            # Update header2
            cat $1 | sed "s/$to_replace/$to_replace_with/g" > temp
            mv temp $1
        fi
        r=$((r+1))
    done
...
model_evaluation(){
    ...
    # ---------------------------
    # Way 2 : Use a predictive model to precisely select corresponding files
...
do
    ...
    if [[ -n "$tot_header_ev" ]] || [[ -n "$header2_ev" ]]; then
        ...
        if [[ "$tot_header_ev" != "$header2_ev" ]]; then
            ...
            exact_match header2 tot_header
        ...
        # ---------------------------
        # Determine if files are in the same table or a different table
        # ---------------------------
...
# [Step 1] Calculate the order for primary keys 
# get_primary_key_list


# [Step 2] Loop through all the csv files and change the columns of data in alphabetical order using tot_header
# organize_csv_columns
