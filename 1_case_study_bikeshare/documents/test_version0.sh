...
initial_setup(){
    ...
    export folder_2_organize=$(echo "data/csvdata_test")
    ...
    ls | sed 's/colBvalue//g' | sed 's/diffrow//g' | sed 's/diff//g' | sed 's/dummy_name//g' | sed 's/file0//g' | sed 's/file1//g' | sed 's/file2//g' | sed 's/file3//g' | sed 's/file4//g' | sed 's/file5//g' | sed 's/file_list//g' | sed 's/filetemp_reevaluate//g' | sed 's/filetemp//g' | sed 's/header2reevaluate_per_row//g' | sed 's/header2//g' | sed 's/header_match_count_per_file//g' | sed 's/header_reduced//g' | sed 's/htemp//g' | sed 's/modelscores//g' | sed 's/new_csv//g' | sed 's/result//g' | sed 's/rows_that_repword_appear//g' | sed 's/sim_float//g' | sed 's/sim_int//g' | sed 's/tempcount//g' | sed 's/to_replace_assignment//g' | sed 's/tot_headerreevaluate_per_row//g' | sed 's/uqtemp//g' | sed 's/test.sh//g'  | sed 's/temp//g' | sed '/^$/d' > file_list
...
do
    ...
    if [[ -n "$tot_header_ev" ]] || [[ -n "$header2_ev" ]]; then
        ...
        # Update tot_header with header2 values 
        cat tot_header header2 | sort -u > temp
        mv temp tot_header
